...
void main() {
    ...
    if (isWater) {
        ...
        if (isWater) {
            vec3 pixelScaledPos = (translucentFeetPlayerPos + cameraPosition) * PIXEL_SIZE;
            translucentFeetPlayerPos = (floor(pixelScaledPos) + normal * sign(pixelScaledPos) + vec3(0.5)) / PIXEL_SIZE - cameraPosition;
            translucentViewPos = (gbufferModelView * vec4(translucentFeetPlayerPos, 1.0)).xyz;
        }
        #endif
        ...
        if (fadeFactor < 1.0) {
            ...
            if (minVec3(shadow) > 0.0 && dot(waveNormal, lightDir) > 0.0) {
                skyReflection += (1.0 - rainStrength) * max0(brdf(material, waveNormal, waveNormal, translucentViewPos, shadow, scatter) * sunlightColor);
            }
            #ifdef CLOUDY_FOG
